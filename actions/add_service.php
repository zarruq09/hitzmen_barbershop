...
// If add_service.php is in 'actions/' and db.php is in the root, '../db.php' is correct.
require '../db.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Duplicate Check
    $stmtCheck = $pdo->prepare("SELECT COUNT(*) FROM services WHERE service_name = ?");
    $stmtCheck->execute([$service_name]);
    if ($stmtCheck->fetchColumn() > 0) {
        header('Location: ../admin_dashboard.php?error=duplicate_service&tab=services');
        exit();
    }
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $targetFilePath)) {
            ...
            error_log("Failed to move uploaded service image: " . $_FILES['image']['tmp_name'] . " to " . $targetFilePath);
            // Redirect with an error message and stay on the services tab
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO services (service_name, price, description, image) VALUES (?, ?, ?, ?)");
        // Execute the prepared statement with the sanitized data
        $success = $stmt->execute([$service_name, $price, $description, $image]);

        if ($success) {
            // Redirect back to the admin dashboard, specifically to the services tab on success
            ...
            error_log("Failed to insert service into database. Info: " . implode(" | ", $stmt->errorInfo()));
            header('Location: ../admin_dashboard.php?error=add_service_failed&tab=services');
            exit();
        }
    } catch (PDOException $e) {
        // Catch any PDO database-related errors
        error_log("Database Error adding service: " . $e->getMessage());
        header('Location: ../admin_dashboard.php?error=db_error&tab=services');
        exit();
    }
} else {
